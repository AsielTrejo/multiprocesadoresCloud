  ...
  #pragma omp parallel
    {
      ...
      #pragma omp for schedule(dynamic)
      for (int k = number/2; k < (alto*ancho);k++){

          //Obtengo la coordenada del fila y columna
          row = k/ancho;
          column = k-(row*ancho);

          if(column+(number/2)<finishLine && row+(number/2)<finishRow && row > number/2 && column>number/2){ //conditions to find center
            //column+=1;
            baseRow = row - (number/2);//Resets to the origin of the square
            auxrow = baseRow;
            baseColumn = column - (number/2);//Resets to the origin of the square
